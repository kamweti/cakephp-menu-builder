...
$(function(){
	...
	$('.add_menu_item').click(function(){
		...
		//build url
		var base_url=$('body').attr('baseurl'); //has no trailing slash
		...
		if(regex.test(_action)==true){
			...
			_linkurl	=base_url+'/admin/'+_controller+'/'+_actioninurl; //prefixes url with admin
		}else{
			_linkurl	=base_url+'/'+_controller+'/'+_action;
		}
